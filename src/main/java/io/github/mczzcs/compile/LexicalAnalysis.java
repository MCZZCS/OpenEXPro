...
public final class LexicalAnalysis {
    ...
    private Token lex() throws NullPointerException {
        ...
        int c;
        do {
            c = getChar(); //每调用一次拉取一个字符,并将下标移到文件下一位,内容读完返回null
        } while (isSpace(c)); //判断字符是否是空格
        if (isSEM(c)) { //判断是否是符号 '_'等除外
            sb.append((char) c);
            return new Token(Token.SEM, sb.toString(), file_line);
        } else if (isNum(c)) { //判断数值
            ...
            do {
                c = getChar();
                if (c == '\n')
                    throw new CompileException("'\"' expected.", new Token(Token.STRING, sb.toString(), file_line), file_name, null);
                if (c == '\\') {
                    c = getChar();
                    if (c == 'n') {
                        sb.append("\n");
                        ...
                        throw new CompileException("Illegal escape character in string literal.", new Token(Token.STRING, sb.toString(), file_line), file_name, null);
                    continue;
                }
                sb.append((char) c);
            } while (c != '"');
